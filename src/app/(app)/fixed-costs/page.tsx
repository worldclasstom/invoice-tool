...
const CATEGORIES = ['utilities', 'employees', 'advertising', 'rent', 'insurance', 'other']
...
export default function FixedCostsPage() {
  ...
  // Pie chart data: payments by method
  const methodTotals: Record<string, number> = {}
  costs.filter((c) => c.is_paid).forEach((c) => {
    methodTotals[c.payment_method] = (methodTotals[c.payment_method] || 0) + Number(c.amount)
  })
  ...
  const totalUnpaid = costs.filter((c) => !c.is_paid).reduce((s, c) => s + Number(c.amount), 0)
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-emerald-500 p-4 shadow-lg shadow-emerald-500/20">
          <p className="text-xs font-medium text-emerald-50/80 uppercase tracking-wide">Paid</p>
          <p className="text-lg font-bold text-white">{formatBaht(totalPaid)}</p>
        </div>
        <div className="rounded-2xl bg-amber-500 p-4 shadow-lg shadow-amber-500/20">
          <p className="text-xs font-medium text-amber-50/80 uppercase tracking-wide">Unpaid</p>
          <p className="text-lg font-bold text-white">{formatBaht(totalUnpaid)}</p>
        </div>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-border bg-card shadow-sm">
        ...
        {isLoading ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {costs.map((cost) => (
                    <tr
                      key={cost.id}
                      className={`border-b border-border/50 last:border-0 transition-all ${
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-muted-foreground">{cost.payment_method}</td>
              ...
              {costs.map((cost) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{cost.name}</p>
                    <p className="text-xs text-muted-foreground capitalize">{cost.category} &middot; {cost.payment_method}</p>
                  </div>
          ...
          <div className="flex h-40 flex-col items-center justify-center gap-2 px-4 text-center">
            <p className="text-sm text-muted-foreground">No fixed costs for this month yet.</p>
          </div>
        )}
      </div>
